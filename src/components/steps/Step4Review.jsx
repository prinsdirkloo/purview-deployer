  ...
  return (
    <div className={s.step}>
      ...
      <div className={s.sectionHead}>
        ...
        <p>
          Enter your tenant details — written directly into the generated XML
          and scripts.
        </p>
      </div>
      ...
      <div className={[s.card, s.tenantCard].join(" ")}>
        ...
        <div className={s.tenantGrid}>
          <div className={s.formSection} style={{ marginBottom: 0 }}>
            <label className={s.formLabel}>Customer Tenant ID</label>
            <input
              type="text"
              value={tenantId}
      ...
      <div className={s.card}>
        <div className={s.reviewSectionLabel}>
          Sensitive Information Types ({selectedSITs.length})
        </div>
        <div className={s.chipList}>
          {selectedSITs.map((sit) => (
            <span key={sit.id} className={s.chip}>
              {sit.name}
            </span>
          ))}
        </div>
      ...
      {selectedPolicies.map((p) => {
        ...
        if (!cfg) return null;
        const modeLabel =
          MODES.find((m) => m.value === cfg.mode)?.label || cfg.mode;
        return (
          <div key={p.id} className={s.card}>
            ...
            <div className={s.reviewRow}>
              ...
              <span className={s.reviewVal}>
                {p.endpointOnly
                  ? "Endpoint (auto)"
                  : (cfg.locs || p.locs).join(", ") || "(none)"}
              </span>
            </div>
            <div className={s.reviewRow}>
              <span className={s.reviewKey}>Low volume</span>
              <span className={s.reviewVal}>
                {cfg.lowMin}–{cfg.lowMax} instances → audit
              </span>
            </div>
            <div className={s.reviewRow}>
              <span className={s.reviewKey}>High volume</span>
              <span className={s.reviewVal}>
                {cfg.highMin}+ instances → alert + tip
              </span>
            </div>
            {!p.endpointOnly && (
              <div className={s.reviewRow}>
